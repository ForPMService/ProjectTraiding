<#
.SYNOPSIS
    Замер производительности ProjectTraiding — все endpoints.
...
.DESCRIPTION
    Адаптация к текущему состоянию репо после Cleanup Handoff v0.1:
    - Убраны Reference (GetStockMarkets/GetFuturesMarkets) — перенесены в _Old
    - Убраны 12 calendar-эндпоинтов — перенесены в _Old, остались stock-offdays и futures-offdays
    - Добавлены instrument-cards/stock и instrument-cards/futures (новые карточки инструментов)
    - ISS-параметры оптимизации (iss.meta=off, iss.only, data.columns) уже в клиенте
    - TTFB, байты, строки без десериализации, GC.Collect, медиана — из Baseline2

.EXAMPLE
    # Окно 1: dotnet run --project backend\src\ProjectTraiding.Api\ProjectTraiding.Api.csproj
    # Окно 2: .\Measure-Baseline2_LogEvidence.ps1 -OutputName "post-iss-optimization"

    # С логами:
    # .\Measure-Baseline2_LogEvidence.ps1 -AppLogPath ".\docs\performance\app-manual.log"
#>

param(
    [string]$BaseUrl = "http://localhost:5025",
    [string]$OutputName = "post-iss-optimization",
    [int]$Iterations = 3,
...
$endpoints = @(
    # === Instrument Cards (ISS / APIM) ===
    @{ Name = "instrument-cards/stock";    Url = "/instrument-cards/stock";    Group = "Instrument Cards" },
    @{ Name = "instrument-cards/futures";  Url = "/instrument-cards/futures";  Group = "Instrument Cards" },
...
Write-Host ""
Write-Host "=== ProjectTraiding Performance v2 ===" -ForegroundColor Cyan
...
# Проверка сервера
try {
    $check = $httpClient.GetAsync("$BaseUrl/instrument-cards/stock").GetAwaiter().GetResult()
    ...
    Write-Host "Запусти: dotnet run --project backend\src\ProjectTraiding.Api\ProjectTraiding.Api.csproj" -ForegroundColor Red
    $httpClient.Dispose()
    exit 1
}
...
if (-not $proc) { $proc = Get-Process -Name "ProjectTraiding.Api" -ErrorAction SilentlyContinue | Select-Object -First 1 }
