...
{
    public sealed class TradeStatsStockLoadHandler : ILoadHandler
    {
        private readonly MoexHttpAlgClient _client;
        private readonly RowWriter<SuperCandlesTradeStats5mDTO> _writer;

        public TradeStatsStockLoadHandler(
            MoexHttpAlgClient client,
            RowWriter<SuperCandlesTradeStats5mDTO> writer)
        {
            _client = client;
            _writer = writer;
        }
        ...
        public bool CanHandle(MoexLoadTask task) =>
            task.DataKind == "tradestats" && task.Market == "stock";
            ...
            MoexLoadTask task, LoadStopOutcome stopOutcome, CancellationToken ct)
        {
            ...
            IAsyncEnumerable<List<SuperCandlesTradeStats5mDTO>> pages =
                _client.GetSuperCandlesTradeStats5m(
        ...
        private static string BuildMethod(MoexLoadTask task) =>
            $"/datashop/algopack/eq/tradestats/{task.Secid}.json";
