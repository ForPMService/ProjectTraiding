...
namespace ProjectTraiding.Moex.Parsing
{
    ...
    public static class ParsingInstrumentCardUtf8
    {
        ...
        // ── Чтение строк securities фьючерсов — 15 колонок из 26 ──
            ...
            ColumnAndNumbersForParsing.ExpectedSchema schema)
        {
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndArray)
            {
                ...
                string? lastTradeDate = null, lastDelDate = null, assetCode = null;
                            ...
                            && pos == schema.Columns[expectedIdx].SourceIndex)
                        {
                            if (reader.TokenType != JsonTokenType.Null)
                            {
                                switch (expectedIdx)
                                {
                                    ...
                                    case 8:  assetCode     = ParseHelpersUtf8.ReadString(ref reader, rowIndex, expectedIdx, schema.RootKey); break; // ASSETCODE [11]
                                    case 9:  lotVolume     = ParseHelpersUtf8.ReadInt(ref reader, rowIndex, expectedIdx, schema.RootKey);    break; // LOTVOLUME [13]
                                    case 10: initialMargin = ParseHelpersUtf8.ReadDouble(ref reader, rowIndex, expectedIdx, schema.RootKey); break; // INITIALMARGIN [14]
                                    case 11: highLimit     = ParseHelpersUtf8.ReadDouble(ref reader, rowIndex, expectedIdx, schema.RootKey); break; // HIGHLIMIT [15]
                                    case 12: lowLimit      = ParseHelpersUtf8.ReadDouble(ref reader, rowIndex, expectedIdx, schema.RootKey); break; // LOWLIMIT [16]
                                    case 13: stepPrice     = ParseHelpersUtf8.ReadDouble(ref reader, rowIndex, expectedIdx, schema.RootKey); break; // STEPPRICE [17]
                                    case 14: buySellFee    = ParseHelpersUtf8.ReadDouble(ref reader, rowIndex, expectedIdx, schema.RootKey); break; // BUYSELLFEE [21]
                                }
                            }
                ...
                list.Add(new FuturesInstrumentCardDTO
                {
                    ...
                    AssetCode = assetCode, LotVolume = lotVolume,
