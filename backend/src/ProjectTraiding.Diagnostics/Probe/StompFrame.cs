...
/// полная семантика content-length не поддерживаются. Заголовок content-length сохраняется
/// в Headers как обычная пара, но его значение не используется для длины тела: границу кадра
/// определяет первый нулевой байт.
///
/// Устройство кадра:
...
///
/// Парсер читает тело любого кадра до нулевого байта. CONNECTED от биржи вопреки стандарту
/// STOMP 1.2 несёт тело около 110 КБ — машинный словарь схемы — и оно корректно захватывается;
/// у RECEIPT тела нет.
/// </summary>
