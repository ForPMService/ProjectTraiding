...
namespace ProjectTraiding.Vitrine.StorageBase.Redis
{
    ...
    /// Смотрит в оперативное хранилище; при промахе, недоступности хранилища или
    /// негодном значении читает базу истины и наполняет кеш со сроком жизни.
    /// Кеш есть ускорение поверх истины: путь к базе при отсутствии данных не отсекается.
    /// </summary>
    public sealed class InstrumentCatalogCache
    {
        ...
        private readonly ILogger<InstrumentCatalogCache> _logger;
        ...
        public async Task<List<VitrineInstrumentDto>> GetAllAsync(CancellationToken ct)
        {
            ...
            List<VitrineInstrumentDto>? cached = await TryReadFromCacheAsync();
        ...
        private async Task<List<VitrineInstrumentDto>?> TryReadFromCacheAsync()
        {
            try
            {
                ...
                List<VitrineInstrumentDto>? list = JsonSerializer.Deserialize(
                    (string)value!, VitrineJsonContext.Default.ListVitrineInstrumentDto);
        ...
        private async Task TryWriteToCacheAsync(List<VitrineInstrumentDto> list)
        {
            try
            {
                ...
                string json = JsonSerializer.Serialize(
                    list, VitrineJsonContext.Default.ListVitrineInstrumentDto);
