...
{
    public sealed class InstrumentRelationBySecidReadQuery
    {
        private readonly NpgsqlDataSource _dataSource;
        private readonly ILogger<InstrumentRelationBySecidReadQuery> _logger;

        public InstrumentRelationBySecidReadQuery(NpgsqlDataSource dataSource, ILogger<InstrumentRelationBySecidReadQuery> logger)
        {
            _dataSource = dataSource;
            _logger = logger;
        }

        public async Task<List<VitrineInstrumentRelationDto>> GetBySecidAsync(string secid, CancellationToken ct)
        {
            ...
            try
            {
                ...
                await using NpgsqlCommand cmd = new NpgsqlCommand("""
                    ...
                    WHERE source_secid = @secid OR target_secid = @secid
                    ORDER BY id
                    """, connection);
