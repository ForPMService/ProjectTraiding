...
using ProjectTraiding.CustomFeatures.Infrastructure;
using ProjectTraiding.CustomFeatures.StorageBase.Postgres;

namespace ProjectTraiding.CustomFeatures.Loading;
...
public sealed class CalendarLoader
{
    ...
    {
        DateOnly effectiveDateTill = dateTill ?? CalendarMoscowTime.Today;
        ...
        for (int rangeIndex = 0; rangeIndex < ranges.Count; rangeIndex++)
        {
            ...
            List<CalendarOffDaysMarketDTO> stockRows =
                await _apimClient.GetStockOffDays(range.From, range.Till, ct);
            List<CalendarOffDaysMarketDTO> futuresRows =
                await _apimClient.GetFuturesOffDays(range.From, range.Till, ct);
        ...
        CalendarBulkWriteResult result = await _calendarWriter.UpsertDaysAsync(output, ct);
    ...
    public async Task<int> LoadIntervalsAsync(CancellationToken ct)
    {
        ...
        List<RfudSecurityDTO> activeFutures =
            await _apimClient.GetActiveRfudSecIds(ct);
        ...
        CancellationToken ct)
    {
        ...
        for (int rangeIndex = 0; rangeIndex < ranges.Count; rangeIndex++)
        {
            ...
            expirations.AddRange(
                await _apimClient.GetFuturesSecurities(range.From, range.Till, ct));
        }

        CalendarBulkWriteResult result = await _referenceWriter.ReplaceExpirationsAsync(
    ...
    public async Task<int> LoadSplitsAsync(CancellationToken ct)
    {
        ...
        CalendarBulkWriteResult result = await _referenceWriter.ReplaceSplitsAsync(
