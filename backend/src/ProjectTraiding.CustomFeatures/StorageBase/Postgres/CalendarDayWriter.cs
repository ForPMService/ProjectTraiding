...
public sealed class CalendarDayWriter
{
    private readonly NpgsqlDataSource _dataSource;
    private readonly ILogger<CalendarDayWriter> _logger;

    public CalendarDayWriter(NpgsqlDataSource dataSource, ILogger<CalendarDayWriter> logger)
    {
        _dataSource = dataSource;
        _logger = logger;
    }

    public async Task<CalendarBulkWriteResult> UpsertDaysAsync(
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            CustomFeaturesWriterLogMessages.BulkWriteCompleted(_logger, table, rowsWritten, elapsed);
            return new CalendarBulkWriteResult(days.Count, rowsWritten, elapsed);
        }
        catch (Exception ex)
        {
            CustomFeaturesWriterLogMessages.BulkWriteRolledBack(
