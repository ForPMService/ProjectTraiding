using ProjectTraiding.Management.Contracts.Dto;
using System;
using System.Collections.Generic;
using System.Text;
using System.Text.Json.Serialization;

namespace ProjectTraiding.Management.Contracts
{
    [JsonSerializable(typeof(BrokerTariffCreateRequest))]
    [JsonSerializable(typeof(InstrumentRelationCreateRequest))]
    [JsonSerializable(typeof(ManagementResultDto))]
    [JsonSerializable(typeof(LoadTaskCreateRequest))]
    [JsonSerializable(typeof(LoadTaskBulkCreateRequest))]
    [JsonSerializable(typeof(LoadTaskBulkCreateResponse))]
    [JsonSerializable(typeof(RealtimeSubscriptionResultDto))]
    [JsonSerializable(typeof(RealtimeSubscriptionsDisableAllResponse))]
    [JsonSerializable(typeof(LoadTasksCancelResponse))]
    [JsonSerializable(typeof(InstrumentDataDeleteAcceptedResponse))]
    [JsonSerializable(typeof(InstrumentDataDeleteStatusResponse))]
    [JsonSerializable(typeof(CalendarLoadRequest))]
    [JsonSerializable(typeof(CalendarDayOverrideRequest))]
    [JsonSerializable(typeof(CalendarOperationResponse))]
    [JsonSerializable(typeof(TradingPeriodCreateRequest))]
    [JsonSerializable(typeof(TradingPeriodBatchCreateRequest))]
    [JsonSerializable(typeof(TradingPeriodBatchCreateResponse))]
    [JsonSerializable(typeof(DividendEventCreateRequest))]
    [JsonSerializable(typeof(DividendEventBatchCreateRequest))]
    [JsonSerializable(typeof(DividendEventBatchCreateResponse))]
    [JsonSerializable(typeof(DividendEventUpdateRequest))]
    [JsonSerializable(typeof(CbRateMeetingFactRequest))]
    [JsonSerializable(typeof(CbRateMeetingFactCreateResponse))]
    [JsonSerializable(typeof(CbRateRateBlock))]
    [JsonSerializable(typeof(CbRateRoisfixBlock))]
    [JsonSerializable(typeof(CbRateRuoniaBlock))]
    [JsonSerializable(typeof(CbRateExpertBlock))]
    [JsonSerializable(typeof(CbRateCalendarDayRequest))]
    [JsonSerializable(typeof(CbRateCalendarBatchRequest))]
    [JsonSerializable(typeof(LoadResultDto))]
    [JsonSerializable(typeof(LoadResultDto[]))]
    [JsonSerializable(typeof(ApiErrorDto))]
    public partial class ManagementJsonContext : JsonSerializerContext;
    
}
